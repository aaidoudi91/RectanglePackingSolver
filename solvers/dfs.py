...
class DFS(SolveurBase):
    ...
    # 1. Vérification et génération de positions
    def _meilleur_bloquant(self, x, y, w, h):
        """ Retourne la coordonnée 'x' de fin du meilleur bloqueur, ou None.
        Au lieu de s'arrêter au premier rectangle qui chevauche, on cherche celui
        qui s'étend le plus loin vers la droite pour maximiser notre saut. """
        meilleur_saut = -1
    ...
    def _positions_candidates_generateur(self, rect):
        """ Génère (yield) les positions candidates à la volée pour économiser la mémoire. """
        limite_x = self.largeur_conteneur - rect.largeur
        limite_y = self.hauteur_conteneur - rect.hauteur

        # Élagage 2 : Brisure de symétrie gérée directement à la source
        if not self.rectangles_places:
            limite_x_sym = limite_x // 2
            limite_y_sym = limite_y // 2

            # Calcul mathématique strict du nombre de positions ignorées par la symétrie
    ...
    def _placer(self, rect, x, y):
        """ Place le rectangle et met à jour les états incrémentaux du conteneur. """
        rect.x = x
        rect.y = y
        self.rectangles_places.append(rect)
        self.aire_libre_courante -= rect.aire()
    ...
    def _enlever(self, rect):
        """ Retire le rectangle et restaure les états incrémentaux. """
    ...
    def _calcule_items(rects, index, orientation):
        ...
        items = {}
        # On parcourt uniquement de l'index courant jusqu'à la fin
    ...
    def _borne_martello_toth(capacites, items, taille_max):
        ...
        for taille in range(1, taille_max + 1):
            bin_area = bins.get(taille, 0)
            item_area = items.get(taille, 0)
            total_items = carryover + item_area

            if bin_area > total_items:
    ...
    def _dfs(self, rects, index, aire_restante):
        ...
        # Élagage 3 : Korf (on passe la liste complète et l'index actuel)
        ...
        # Exploration via le générateur paresseux
